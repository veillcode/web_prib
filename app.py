...
def build_user_response(user, profile, db):
    total_files  = len([f for f in db['files'] if f['userId'] == user['id']])
    today_uploads = len([f for f in db['files']
                         if f['userId'] == user['id'] and f.get('uploadedAt', '').startswith(today_str())])
    total_size   = sum(f.get('size', 0) for f in db['files'] if f['userId'] == user['id'])
    return {
        'id':        user['id'],
...
def change_account():
    ...
    if username:
        if len(username) < 3:
            return jsonify({'error': 'Username minimal 3 karakter.'}), 400
        if any(u['username'].lower() == username.lower() and u['id'] != user['id'] for u in db['users']):
            return jsonify({'error': 'Username sudah digunakan.'}), 409
